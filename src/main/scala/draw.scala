...
trait draw extends update {
  ...
  def drawLine(x1: Int, y1: Int, x2: Int, y2: Int)(canvas: Canvas) = {
    def drawLineHelper(xa: Int, ya: Int, xb: Int, yb: Int)(canvas1: Vector[Vector[Char]]): Vector[Vector[Char]] = try {(xa, ya, xb, yb) match {
